...
All entities are created equal but may later specify new bahaviors and capabilities, so the rendering and physics pipelines rely on *duck typing* to determine a game entity's behavior and capability. If an entity lacks certain capability that a pipeline aims to handle, it won't get processed by the pipeline until it obtains the required capability.
