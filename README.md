...
**loader.js** - handles the loading of modules and game assets. Incorporates a custom, high performance asynchronous module loading pattern. It's somewhat like RequireJS/AMD without the ceremony. Maybe it's somewhat like CommonJS without the imposed global namespace, mamed debugging, or build step. Simply create *your own* namespace for *your own* app and pass it to the loader's constructor. No wrappers, no boilerplate, no module definitions, no eval(). If a module class contains a "requires" var, those dependencies get loaded with the module before the module fires its onloadcomplete callback or init() function. This behavior is automated and recursive to support any number of sub-dependencies. If the same module is requested twice that's fine, the second request will pull it straight from the cache without making an HTTP request.

Supports loading scripts and other resources in parallel or in series on-demand, dynamically at run-time (granular lazy loading). This is very important for games that may consist of several gigabytes of data, which need to be shifted into and out of the browser as needed at runtime. Utilizes script, style and link tags when possible to allow cross-domain/CDN resource loading. Utilizes XMLHttpRequest/CORS for other types of resources. The loader currently supports .js .json .css .woff .woff2 .jpg .jpeg .bmp .gif .mp3 and of course http for CDN/Google Web Fonts.

There is a separate loader for game assets that works similarly, but allows .json files to act like relational data (not unlike MongoDB) for designing re-usable game objects that incorporate any number of properties and data from other assets and components. This may get merged into a single loader.
